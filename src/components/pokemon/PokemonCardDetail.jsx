...
import { getGradientClass } from "@/constants/pokemonColor"
...
const styles = {
  ...
  // 컨텐츠 스타일
  iconContainer: "mb-4 flex items-center justify-between gap-1 relative z-[1]",
  cardOwner: "flex items-center text-xl font-bold text-gray-800 relative z-[1]",
 ...
 * 포켓몬 카드 상세 정보 컴포넌트
 * @param {object} props
 * @param {object} props.data - 카드 데이터
 * @param {string} [props.className] - 추가 클래스
 * @param {boolean} [props.showCardCode] - 카드 코드 표시 여부
 * @param {boolean} [props.hideInfo] - 상세 정보 숨김 여부
 * @param {ReactNode} [props.customImageWrapper] - 커스텀 이미지 래퍼
 * @param {ReactNode} [props.customDetailsContainer] - 커스텀 상세 컨테이너
 */
export default function PokemonCardDetail({ 
  ...
  return (
    <div className={`${styles.container} ${styles.root} ${className}`}>
      <div className={hideInfo ? styles.infoHideGrid : styles.grid}>
        {/* 왼쪽: 카드 이미지*/}
