...
export default function TradeItem ({id, card, isActiveCard, disabled, handleClick}) {
  ...
  return (
    <motion.div
      layoutId={`card-${card.code}-${id}`}
      key={`card-${card.code}-${id}`}
    ...
    >
      <div className="flex gap-4 flex-col md:flex-row">
        <motion.div layoutId={`image-${card.code}-${id}`} className="flex justify-center items-center">
